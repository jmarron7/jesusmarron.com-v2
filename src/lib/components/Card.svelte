...
<div class="group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50">
  ...
  <div class="z-10 mb-2 mt-1 text-sm font-semibold uppercase tracking-wide text-surface-300 sm:col-span-2">
    <img src={"images/" + data.imagePath} alt={"Preview image of " + data.title} loading="lazy" width="200" height="48" decoding="async" class="rounded border-2 border-success-800/90 transition group-hover:border-success-500 sm:order-1 sm:col-span-2 " />
  </div>
  {/if}
  <div class="z-10 sm:col-span-6">
    <h3 class="font-medium leading-snug">
      <div>
        {#if isExperience(data)}
        <a class="inline-flex items-baseline font-semibold leading-tight hover:text-success-500 focus-visible:text-success-500 group/link text-base" href="{data.companyUrl}" target="_blank" rel="noreferrer noopener" aria-label="{data.role} at {data.company} (opens in a new tab)">
        ...
        {:else if isProject(data)}
        <span class="font-semibold leading-tight group-hover:text-success-500 focus-visible:text-success-500 group/link text-base" aria-label="{data.title}">
          {data.title}
        </span>
        {/if}
      </div>
    ...
    <ul class="mt-2 flex flex-wrap" aria-label="Technologies used">
      ...
      <li class="mr-1.5 mt-2">
        <div class="flex items-center rounded-full bg-success-400/10 px-3 py-1 text-sm font-medium leading-5 text-success-300 ">
          {tech}
        </div>
      </li>
      {/each}
